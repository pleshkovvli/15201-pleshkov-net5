...
typedef enum {IN_PROGRESS, TO_DO} task_status;
        ...
        for(int sock_num = 0; sock_num < number_of_sockets && events > 0; ++sock_num) {
            ...
            if(cur_poll->events == POLLIN) {
                ...
                if(sock_num == 0) {
                    ...
                    if((cur_client->state == UNKNOWN) && (cur_client->bytes_read >= MSG_LEN)) {
                        if(memcpy(cur_client->buffer, register_msg, MSG_LEN) == 0) {
                            cur_client->state = REGISTERED;
                        } else if(memcmp(cur_client->buffer, success_msg, MSG_LEN) == 0) {
                            cur_client->state = SUCCESS;
                            success = TRUE;
                            success_num = sock_num;
                        } else if(memcmp(cur_client->buffer, more_msg, MSG_LEN) == 0) {
                            cur_client->state = MORE;
                        } else {
                            fprintf(stderr, "Unknown message type");
                            --number_of_sockets;
                            if(sock_num < number_of_sockets) {
                                cur_poll->fd = socket_polls[number_of_sockets].fd;
                                cur_poll->events = socket_polls[number_of_sockets].events;
                                cur_poll->revents = socket_polls[number_of_sockets].revents;
                                continue;
                            }
                        }
                    }

                    if(cur_client->state == REGISTERED) {
                        if((cur_client->bytes_read >= CONF_LEN)) {
                            memcpy(tasks[tasks_number].uuid, &(cur_client->buffer[MSG_LEN]), UUID_LEN);
                            ...
                            tasks[tasks_number].status = IN_PROGRESS;
                            ++tasks_number;
                    ...
                    if(cur_client->state == MORE) {
                        if((cur_client->bytes_read >= 20)) {
                            int j = 0;
                    ...
                    if(cur_client->state == SUCCESS) {
                        if((cur_client->bytes_read >= 20)) {
                            int j = 0;
                            for(; j < tasks_number; ++j) {
                                if(strncmp(tasks[j].uuid, (char*)  &clients[j].buffer[4], 16) == 0) {
                                    break;
                                }
                            }
                            if(j == tasks_number) {
